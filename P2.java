...
private void movePlayerQ(int n){
    ...
    public void act()
    {
      ...
      GreenfootImage image = getImage();
       ...
       if (Greenfoot.mouseClicked(this)&&turnTwo==true){
         diceRolls = getWorld().getObjects(Dashboard.class).get(0).getDiceRollsPlayerTwo();          
         movePlayer();  
    }
    numRolls = getWorld().getObjects(Dice.class).get(0).getNumRolls()+getWorld().getObjects(DiceTwo.class).get(0).getNumRolls2();
    if(numRolls==4||numRolls==8||numRolls==12||numRolls==16||numRolls==20){
        turnTwo=true;
    }
    else{
        turnTwo=false;
